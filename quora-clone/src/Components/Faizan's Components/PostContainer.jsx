...
function PostContainer(props) {
    ...
    const { postuser, setinputdata, postdata, inputdata,toggle,settoggle ,dark, setdark } = useContext(AuthContext);
    ...
    useEffect(() => {
        ...
        const revarray=postdata.reverse();
    ...
    return (
        ...
        <>
            ...
            {revarray?.map((elem) => {
                return <>
                    {/* <h1>{elem.des}</h1> */}
                    <Box bg = {dark? 'black':'null'}
                color={!dark? 'black':'white'} mt="1rem" mb="2rem" border="0.1px solid lightgray">
                ...
                <Box display="flex" p="0.2rem 0.5rem" alignItems="center">
                    <Box display="flex" p="0.2rem 0.5rem" alignItems="center" gap="0.8rem">
                        <ButtonGroup size='sm' isAttached variant='outline'>
                            <Button onClick={togglefun} borderRadius='5rem' border="1px solid silver" bg={toggle?'blue.200': dark? 'black': 'white'} leftIcon={<RxThickArrowUp />}>
                                <Text fontSize="xs">Upvote</Text>
                            </Button>
                            <IconButton onClick={setdownfun} border="1px solid silver" bg={down?'red.200': dark? 'black': 'white'  } borderRadius='5rem' icon={<RxThickArrowDown />} />
                        </ButtonGroup>
